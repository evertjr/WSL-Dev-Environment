...
- NOTE ABOUT NPM INSTALLS (EX: ESLINT --INIT): For some reason VS Code locks the project folders so npm can't rename folders while VS Code is open, so **you need to close VS Code before running npm installs**.  
